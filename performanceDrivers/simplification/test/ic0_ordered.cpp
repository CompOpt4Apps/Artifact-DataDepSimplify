...
int main(int argc, char *argv[]) {
 ...
 cout<<"\n\n-- finalename = "<<f1<<"  nt = "<<numThread<<"  IP = "<<innerPart<<" DR = "<<divRate<<"\n\n";
...
// std::cout <<avg<<"," <<duration1 << ",";
 ...
 /// Copying for parallel runs
 double *x_par = new double[A2->nzmax];
 int *p_par = new int [n+1];
 int *i_par = new int [A2->nzmax];

 double *x_Hpar = new double[A2->nzmax];
 int *p_Hpar = new int [n+1];
 int *i_Hpar = new int [A2->nzmax];

 for (int i = 0; i < n+1; ++i) {
  p_par[i] = A2->p[i];
  p_Hpar[i] = A2->p[i];
 }
 for (int i = 0; i < A2->nzmax; ++i) {
  i_par[i] = A2->i[i];
  i_Hpar[i] = A2->i[i];
  x_par[i] = A2->x[i];
  x_Hpar[i] = A2->x[i];
...
/*
std::cout<<"\n\n-- Running with inner m-loop parallel :\n";
 //***************Parallel inner
 for (int l = 0; l < iterNo; ++l) {
  start = std::chrono::system_clock::now();
  ic0_csr_parInner(n, A2->x , A2->p, A2->i);
  end = std::chrono::system_clock::now();
  elapsed_seconds = end - start;
  duration1 = elapsed_seconds.count();
   std::cout << "    Duration = "<< duration1 << "\n\n";
 }
...
//                             levelPtr, levelSet);
  ...
  levels = buildLevelSet_CSC(n, A2->nzmax, A2->p, A2->i,
    ...
    //DAG.resize(n);
    // Running the inspector based on dependences 
    //computeDAG_Test(n,A2->p,A2->i, DAG);
    // Building the level sets
...
std::cout<<"\n\n-- Running wavefront parallel using H level set:\n";
 ...
 for (int l = 0; l < iterNo; ++l) {
  start = std::chrono::system_clock::now();
  ic0_csr_hlevel_parallel(n, x_Hpar, p_Hpar, i_Hpar,
                          nLevels,HLevelPtr,HLevelSet,
